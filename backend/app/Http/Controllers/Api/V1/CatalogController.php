...
class CatalogController extends Controller
{
    ...
    public function index(Request $request,$type) { $model=$this->model($type); $items=$model::query()->when($type !== 'units',function($q){return $q->where('is_active',true);})->when($request->q,function($q,$v){return $q->where('name','like','%'.$v.'%');})->orderBy('name')->paginate(min(max((int)$request->input('per_page',15),1),100)); return response()->json(['success'=>true,'data'=>BasicResource::collection($items)]); }
